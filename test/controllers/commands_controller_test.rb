...
class CommandsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create command" do
    assert_difference('Command.count') do
      post commands_url, params: { command: { condition: @command.condition, name: @command.name, status: @command.status } }
    end
  ...
  test "should update command" do
    patch command_url(@command), params: { command: { condition: @command.condition, name: @command.name, status: @command.status } }
